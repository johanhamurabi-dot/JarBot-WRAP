...
st.markdown("""
<style>
.stApp { background: linear-gradient(135deg, #eef7ff 0%, #f7fbff 45%, #ffffff 100%); }
[data-testid="stSidebar"] { background: linear-gradient(180deg, #07111F 0%, #102A43 100%); }
[data-testid="stSidebar"] h1,
[data-testid="stSidebar"] h2,
[data-testid="stSidebar"] h3,
[data-testid="stSidebar"] label,
[data-testid="stSidebar"] p { color: white !important; }
[data-testid="stSidebar"] input { color: #111111 !important; background-color: white !important; }
[data-testid="stSidebar"] [data-baseweb="select"] div { color: #111111 !important; }
.hero {
    background: linear-gradient(135deg, rgba(0,150,255,.90), rgba(0,220,180,.82)),
    url("https://images.unsplash.com/photo-1581093458791-9d15482442f6?auto=format&fit=crop&w=1600&q=80");
    background-size: cover; background-position: center; padding: 38px; border-radius: 28px;
    color: white; box-shadow: 0 14px 34px rgba(0,0,0,.18); margin-bottom: 25px;
}
.hero-title { font-size: 46px; font-weight: 900; margin-bottom: 5px; }
.hero-subtitle { font-size: 18px; font-weight: 500; }
.status-pill { display: inline-block; background: rgba(255,255,255,.22); padding: 8px 16px; border-radius: 999px; font-weight: 700; margin-top: 14px; backdrop-filter: blur(8px); }
.card { background: white; padding: 24px; border-radius: 22px; box-shadow: 0 8px 24px rgba(20,40,80,.10); border: 1px solid #E6EAF0; }
.metric-big { font-size: 44px; font-weight: 900; color: #07111F; }
.metric-label { font-size: 15px; color: #64748B; font-weight: 700; }
...
.stTabs [data-baseweb="tab"] { background: white; border-radius: 16px; padding: 12px 18px; font-weight: 800; border: 1px solid #E6EAF0; }
.stTabs [aria-selected="true"] { background: linear-gradient(135deg, #0096FF, #00D4A6) !important; color: white !important; }
.stDownloadButton button { border-radius: 14px; background: linear-gradient(135deg, #0096FF, #00D4A6); color: white; border: none; font-weight: 800; }
</style>
...
def traditional_2l_rpm_from_graph(G, T):
    """
    Traditional 2 L method based on the Phipps & Bird / B-KER chart.
    Log-log empirical approximation:
        G = 300 * (RPM / RPM_300(T))^n
        RPM = RPM_300(T) * (G/300)^(1/n)
    RPM_300(T) values are approximated from the chart.
    """
    T_points = np.array([4, 10, 16, 22], dtype=float)
    rpm_300_points = np.array([242, 234, 225, 215], dtype=float)
    T_clamped = np.clip(T, 4, 22)
    rpm_300 = np.interp(T_clamped, T_points, rpm_300_points)
    n = 1.25
    rpm = rpm_300 * (G / 300) ** (1 / n)
...
with tab3:
    ...
    fig.update_layout(title=f"3D Calibration Surface — {method}", scene=dict(xaxis_title="G-Value (s⁻¹)", yaxis_title="Temperature (°C)", zaxis_title="RPM"), height=720, template="plotly_white")
    st.plotly_chart(fig, use_container_width=True)
