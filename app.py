...
# Traditional chart data digitized from Phipps & Bird graph
...
def traditional_2l_rpm_from_graph(G, T):
    ...
    rpm_vs_temp = []
    for temp in [4,10,16,22]:
        rpm_vs_temp.append(np.interp(G, G_POINTS, RPM_TABLE[temp]))

    rpm = np.interp(T_clamped, [4,10,16,22], rpm_vs_temp)
...
def g_from_rpm_traditional_2l(rpm, T):
    ...
    g_vs_temp = []
    for temp in [4,10,16,22]:
        g_vs_temp.append(np.interp(rpm, RPM_TABLE[temp], G_POINTS))

    return np.interp(T_clamped, [4,10,16,22], g_vs_temp)
